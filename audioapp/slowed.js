...
export default class SlowedController extends BaseController {
      ...
      <div class="min-h-screen bg-[#D1D5DB] text-white font-['Special_Elite'] text-[12px] overflow-hidden relative flex items-center justify-center p-8 transition-colors duration-300 ${slimeActive ? "bg-[#DCFCE7]" : ""}" data-role="dropzone">
        <div class="w-full max-w-6xl mx-auto p-6 pt-12 relative z-10">
          <div class="flex justify-between items-center mb-6">
            <div class="flex gap-2">
              <button data-action="upload" class="px-6 py-3 bg-[#FCA311] border-2 border-[#B37200] text-[#14253D] text-[12px] font-bold uppercase tracking-widest hover:brightness-110 transition-all shadow-[0_4px_0_0_#B37200] active:translate-y-1 active:shadow-none">
          ...
          <div class="flex flex-col gap-4">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div class="bg-[#F8FAFC] border-4 border-[#14253D] p-4 relative overflow-hidden flex flex-col justify-between min-h-[220px]">
                <div class="absolute top-2 right-2 flex gap-1">
                  <div class="w-2 h-2 bg-[#14253D] rounded-full animate-ping"></div>
                  <div class="w-2 h-2 bg-[#FCA311] rounded-full"></div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-2 text-[8px]">
                  <div class="border border-[#14253D]/30 p-1">
                    <p class="text-slate-500 mb-0.5">TIME</p>
                    <p class="text-[#FCA311] font-bold" data-role="current-time">${formatSeconds(this.currentTime)}</p>
                  </div>
                  <div class="border border-[#14253D]/30 p-1">
                    <p class="text-slate-500 mb-0.5">DUR</p>
                    <p class="text-[#FCA311] font-bold" data-role="duration">${formatSeconds(this.duration)}</p>
                  </div>
                  <div class="border border-[#14253D]/30 p-1">
                    <p class="text-slate-500 mb-0.5">RATE</p>
                    <p class="text-[#FCA311] font-bold">${Math.round(this.slowed * 100)}%</p>
                  </div>
                </div>
              </div>
              <div class="bg-[#F8FAFC] border-4 border-[#14253D] p-4">
                <p class="text-[8px] text-[#14253D] mb-3 uppercase tracking-widest font-bold">Parameter Matrix</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div class="bg-[#F8FAFC] border-4 border-[#14253D] p-4 flex flex-col justify-between">
                <p class="text-[8px] text-[#14253D] mb-3 uppercase tracking-widest font-bold">Transport Control</p>
                ...
                <button data-action="export" ${!this.audioBuffer ? "disabled" : ""} class="relative group flex flex-col items-center justify-center w-full border-4 transition-all duration-75 font-['Special_Elite'] bg-[#FCA311] border-[#B37200] text-[#14253D] shadow-[0_4px_0_0_#B37200] hover:brightness-110 active:translate-y-1 active:shadow-none disabled:opacity-20 disabled:cursor-not-allowed py-3 mt-2">
                  ${icon("download", 18, "mb-1")}
                  <span class="text-[10px] font-bold">EXPORT_MASTER_WAV</span>
                </button>
              </div>
              <div class="bg-[#F8FAFC] border-4 border-[#14253D] p-4 flex flex-col justify-center">
                <p class="text-[#14253D] uppercase tracking-widest mb-2 font-bold text-[8px]">Engine Status:</p>
                <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-slate-600 text-[7px]">
  ...
  toggleSwitch(labelLeft, labelRight, activeRight) {
    return `
      <div class="flex items-center gap-3">
        <span class="text-[10px] font-bold uppercase tracking-wider transition-colors ${!activeRight ? "text-[#FCA311]" : "text-gray-400"}">${labelLeft}</span>
        <button data-action="toggle-mode" class="relative w-10 h-5 rounded-full p-1 transition-colors duration-300 focus:outline-none" style="background-color:${activeRight ? "#FCA311" : "#D1D5DB"}">
        ...
        <span class="text-[10px] font-bold uppercase tracking-wider transition-colors ${activeRight ? "text-[#FCA311]" : "text-gray-400"}">${labelRight}</span>
      </div>
    `;
  }
    ...
    return `
      <div class="space-y-2">
        ...
        <div class="relative h-1 bg-gray-200 rounded-full">
          <div class="absolute top-0 left-0 h-full bg-[#FCA311] rounded-full" style="width:${percentage}%"></div>
          <input class="absolute inset-0 w-full h-full opacity-0 cursor-pointer slider-reset" data-slider="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${value}" />
          <div class="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-white border border-gray-300 rounded-full shadow-sm pointer-events-none" style="left:calc(${percentage}% - 8px)"></div>
        </div>
      </div>
    `;
  }
  ...
  arcadeSlider(label, key, value, min, max, step, displayValue) {
    return `
      <div class="bg-white border border-[#14253D]/30 p-3 space-y-2">
        <div class="flex justify-between items-center">
          <label class="text-[9px] font-bold uppercase tracking-widest text-[#14253D]">${label}</label>
          <span class="text-[9px] font-bold text-[#FCA311] bg-[#FCA311]/10 px-1.5 py-0.5" data-role="${key}-value">${displayValue}</span>
        </div>
        <input class="w-full slider-reset arcade-range" data-slider="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${value}" />
      </div>
    `;
  }
  ...
  arcadeButton(label, action, iconHtml, active = false, disabled = false) {
    return `
      <button data-action="${action}" ${disabled ? "disabled" : ""} class="relative group flex flex-col items-center justify-center w-full border-4 transition-all duration-75 font-['Special_Elite'] ${active ? "bg-white border-[#FCA311] translate-y-1 shadow-none text-[#14253D]" : "bg-[#14253D] border-[#0A121F] text-white shadow-[0_4px_0_0_#0A121F] hover:brightness-110 active:translate-y-1 active:shadow-none"} ${disabled ? "opacity-20 cursor-not-allowed grayscale" : "cursor-pointer"} !aspect-auto py-2">
  ...
  drawWaveform() {
    ...
    drawWaveform(this.waveformCanvas, this.audioBuffer, {
      background: "#FFFFFF",
      waveColor: this.uiMode === "macos" ? "#CBD5E1" : "#14253D",
